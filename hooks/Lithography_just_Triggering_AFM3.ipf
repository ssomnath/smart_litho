...
Function MakeMasterLithoPanel()
	...
	Variable FontSize = ARGetFontSize(11)
...
Function MakeLithoStepPanel(Var)		//make the litho panel
	...
	Variable FontSize = ARGetFontSize(12)
	
	
	String ControlName = ""
	String ParmName = "DeflectionSetPointVolts"
	if (GV("ImagingMode") == 1)
		ParmName = "AmplitudeSetPointVolts"
	endif
	...
	//Mode popup
	if (2^bit & Control1Bit)
		ControlName = "LithoModePopup"+TabStr
		UpdatePopup(GraphStr,ControlName,"Litho Mode",45,CurrentTop,"LithoPopupFunc","\"Deflection;AC Mode;\"",GV("ImagingMode")+1,Enab)
...
Function DoStepLitho()
	...
	Variable Setpoint = GV("DeflectionSetpointVolts")
	Variable Pgain = (GV("ProportionalGain")/10)//10^(GV("ADCgain")/20)
	Variable Igain = (GV("IntegralGain")*100)//10^(GV("ADCgain")/20)
	Variable Sgain = (GV("SecretGain")/1e+12)//10^(GV("ADCgain")/20)
	Variable XPgain = XLVDTSens*10^GV("XPGain")
	Variable XIgain = XLVDTSens*10^GV("XIGain")
	...
	ErrorStr += num2str(ir_WriteValue("A%Output",0))+","
	LithoXPTFunc(1)
	
	
	if (imagingMode == 1)		//if we are in voltage mode and AC mode then use amplitude
		Setpoint = GV("AmplitudeSetpointVolts")
		ErrorStr += num2str(ir_SetPISLoop(2,"Always","R%Input",Setpoint,-PGain,-Igain,-Sgain,"Z%output"))+","
	else
		ErrorStr += num2str(ir_SetPISLoop(2,"Always","Fast%Input",Setpoint,PGain,IGain,SGain,"Z%output"))+","
	endif
...
//	ErrorStr += num2str(ir_SetPISLoop(2,"Always","Fast%Input",Setpoint,PGain,IGain,SGain,"Z%output"))+","
	...
	Variable Xstart = (td_ReadValue("XSensor")-XOffset/XLVDTSens-xLVDTOffset)		//calculate where the XY actually is with the
	Variable Ystart = (td_ReadValue("YSensor")-YOffset/YLVDTSens-yLVDTOffset)		//offset that we will be using
	//set the X and Y feedback loops
	ErrorStr += num2str(ir_SetPISLoop(3,"1","XSensor",XStart,0,XIgain,XSgain,"X%output"))+","
	ErrorStr += num2str(ir_SetPISLoop(4,"1","YSensor",YStart,0,YIGain,YSgain,"Y%output"))+","

	//this is the X & Y offset
	ErrorStr += num2str(ir_WriteValue("SetpointOffset%PISLoop3",XOffset/XLVDTSens+xLVDTOffset))+","
	ErrorStr += num2str(ir_WriteValue("SetpointOffset%PISLoop4",YOffset/YLVDTSens+yLVDTOffset))+","
	...
	LithoStepVoltY[] = (Imag(P2Rect(AngleWave[P])))/YLVDTSens//+YLVDTOffset
	LithoStepVoltX[] = (Real(P2Rect(AngleWave[P])))/XLVDTSens//+XLVDTOffset
...
//	LithoStepVoltX = (LithoStepX-ScanSize/2)/xLVDTSens
	...
	Variable DoX = MakeRamp(0,"Setpoint%PISLoop3",XStart,(LithoStepVoltX[0]),ScanSpeed/(XLVDTSens*4),1,rampInterpX)	//now with a 5um/S lowest speed
	Variable DoY = MakeRamp(1,"Setpoint%PISLoop4",YStart,(LithoStepVoltY[0]),ScanSpeed/(YLVDTSens*4),1,rampInterpY)
	if (DoX || DoY)							//check if we have to run the ramp
		...
		ErrorStr += IR_XSetInWave(0,"0","Fast%Input",DummyWave,"ActualStepLitho(0)",rampInterp)
		//start the ramp
		ErrorStr += num2str(td_WriteString("0%Event","Once"))+","
	else
		ErrorStr += num2str(ir_WriteValue("Setpoint%PISloop3",LithoStepVoltX[0]))+","
		ErrorStr += num2str(ir_WriteValue("Setpoint%PISloop4",LithoStepVoltY[0]))+","
...
Function ActualStepLitho(LithoCount)			//this does the actual lithography
	...
	if (UseStepWave)
		if (UseBias)
			//set the litho bias
			ErrorStr += num2str(ir_WriteValue("A%Output",LithoVolts[LithoCount]))+","
		else
			//set the litho setpoint
			ErrorStr += num2str(ir_WriteValue("Setpoint%PISLoop2",LithoVolts[LithoCount]))+","
		endif
	else
		//set the litho bias
		if (UseBias)
			ErrorStr += num2str(ir_WriteValue("A%Output",GV("LithoStartVolts")))+","
		endif
		//set the litho setpoint
		ErrorStr += num2str(ir_WriteValue("Setpoint%PISLoop2",LithoSetpoint))+","
	endif
...
//this calls the next part	
	...
	//this calls the next part	
	ErrorStr += IR_XSetInWavePair(0,"0","Fast%Input",Deflection,"ZSensor",Height,"LithoStepRamp("+num2str(LithoCount+1)+")",Decimation)
	ErrorStr += IR_XSetInWavePair(1,"0","A%Input",Lateral,"R%Input",Amplitude,"",Decimation)
	ErrorStr += IR_XSetInWave(2,"0","B%Input",Current,"",Decimation)
...
Function LithoStepRamp(LithoCount)		//this function moves between litho segments
	...
	if (ImagingMode)
		...
		Setpoint = GV("DeflectionSetpointVolts")
	endif
	
	ErrorStr += num2str(ir_WriteValue("Setpoint%PISLoop2",Setpoint))+","
	...
	Sum = td_ReadValue(DetectorSumAlias())
	ErrorStr += num2str(td_StopOutWaveBank(-1))+","
...
//	ErrorStr += IR_StopInWaveBank(-1)
	...
	XSpeed = (ScanSpeed/(XLVDTSens*4))
	YSpeed = (ScanSpeed/(YLVDTSens*4))
	XTime = XDist/XSpeed
	YTime = YDist/YSpeed
	if (XTime > YTime)
		YTime = XTime
	...
	Variable DoX = MakeRamp(0,"Setpoint%PISLoop3",LithoStepVoltX[LithoCount-1],LithoStepVoltX[LithoCount],XSpeed,1,rampInterpX)	//now with a 5um/S lowest speed
	Variable DoY = MakeRamp(1,"Setpoint%PISLoop4",LithoStepVoltY[LithoCount-1],LithoStepVoltY[LithoCount],YSpeed,1,rampInterpY)
	if (DoX || DoY)										//do we need to ramp?
		...
		ErrorStr += IR_XSetInWave(0,"0","Fast%Input",DummyWave,"ActualStepLitho("+num2str(LithoCount)+")",rampInterp)
		ErrorStr += num2str(td_WriteString("0%Event","Once"))+","
	else
		ErrorStr += num2str(ir_WriteValue("Setpoint%PISloop3",LithoStepVoltX[LithoCount]))+","
		ErrorStr += num2str(ir_WriteValue("Setpoint%PISloop4",LithoStepVoltY[LithoCount]))+","
...
//	IsStopDraw = ItemsInList(FindControls2(CtrlName2Find,-1,PassByRefStr),";")
	...
	ErrorStr += num2str(td_StopOutWaveBank(-1))+","
	ErrorStr += IR_StopInWaveBank(-1)
	ErrorStr += num2str(td_WriteString("XDestination%ScanEngine","Dummy%Output"))+","
	ErrorStr += num2str(td_WriteString("YDestination%ScanEngine","Dummy%Output"))+","
	...
	if (LithoSnap)
		ZStr = "Setpoint%PISLoop2"
		if (TriggeredSnap)
			ErrorStr += num2str(ir_SetPISLoop(1,"3,2","Fast%Input",Setpoint,PGain,IGain,SGain,"Z%output"))+","
			EventStr = "2,3"
		endif
		ErrorStr += num2str(ir_SetPISLoop(2,EventStr,"ZSensor",td_RV("ZSensor"),0,10^GV("ZIGain")*GV("zLVDTSens"),0,"Z%output"))+","
...
//		ErrorStr += num2str(ir_WriteValue(ZStr,1))+","
	...
	elseif (imagingMode == 1)		//if we are in voltage mode and AC mode then use amplitude
		Setpoint = GV("AmplitudeSetpointVolts")
		ErrorStr += num2str(ir_SetPISLoop(2,"Always","R%Input",Setpoint,-PGain,-Igain,-Sgain,"Z%output"))+","
	else
		ErrorStr += num2str(ir_SetPISLoop(2,"Always","Fast%Input",Setpoint,PGain,IGain,SGain,"Z%output"))+","
	...
	ErrorStr += num2str(ir_StopPISLoop(3))+","
	ErrorStr += num2str(ir_StopPISLoop(4))+","
...
//	Variable XStart = td_ReadValue("XSensor")
	Variable XStop = imag(tempBoth)/XLVDTSens
	Variable XSpeed = abs(ScanSpeed/(XLVDTSens*4))
	Variable XDist = abs(XStop-XStart)
	Variable XTime = XDist/XSpeed
	
//	Variable YStart = td_ReadValue("YSensor")
	Variable YStop = real(tempBoth)/YLVDTSens
	Variable YSpeed = abs(ScanSpeed/(YLVDTSens*4))
	...
	ErrorStr += num2str(td_SetRamp(Min(max(XTime,YTime)+.1,10),"Setpoint%PISLoop3",XSpeed,XStop,"SetPoint%PISLoop4",YSpeed,YStop,ZStr,ZSpeed,ZDest,"ActualLitho(0)"))+","
...
Function LithoBackground()
	...
	tempY = (td_ReadValue("YSensor")-MVW[%YLVDTOffset])*MVW[%YLVDTSens][0]-RVW[%YOffset][0]//+LVW[%LithoScanSize]/2
	tempX = (td_ReadValue("XSensor")-MVW[%XLVDTOffset])*MVW[%XLVDTSens][0]-RVW[%XOffset][0]//+LVW[%LithoScanSize]/2
...
Function CheckLithoDisplay()
	...
	if (((2^(WhichListItem("Height",MDTL)-1) & dataTypeSum) == 0) && (2^(WhichListItem("ZSensor",MDTL)-1) & dataTypeSum))	//if there is no height but there is ZSensor
...
//print td_RV("ZSensor")*-GV("ZLVDTSens"),GetFuncName()
	...
	Variable PPS = 1e5/Interpolation
	
	FastOp DriveY = (1/YLVDTSens)*DriveY
	FastOp DriveX = (1/XLVDTSens)*DriveX
...
//			CTFCParms[%RampSlope1][0] = num2str(RampDist/RampTime)
			...
			CTFCParms[%RampChannel][0] = "Dummy%output"
			CTFCParms[%RampOffset1][0] = num2str(RampDist)
			CTFCParms[%RampSlope1][0] = num2str(RampDist/RampTime)

			CTFCParms[%RampOffset2][0] = "0"
			CTFCParms[%RampSlope2][0] = "0"
			CTFCParms[%TriggerChannel1][0] = "Fast%Input"
//			CTFCParms[%TriggerValue1][0] = num2str(td_ReadValue("Fast%Input")+TriggerValue*TriggerSlope)
			CTFCParms[%TriggerValue1][0] = num2str(Setpoint)
			CTFCParms[%TriggerCompare1][0] = ">="
			CTFCParms[%TriggerChannel2][0] = "Fast%Input"
...
//		else
			ErrorStr += num2str(td_xSetOutWave(1,"0","Setpoint%PISloop2",LithoSnapDrive,-Interpolation))+","
...
//		endif
	...
	elseif (UseWave)
		...
		if (UseBias)
			ErrorStr += num2str(td_xSetOutWave(1,"0","A%output",DriveVolts,Interpolation*InterpSign))+","
			LithoBias = DriveVolts[0]
		else
			ErrorStr += num2str(td_XSetOutWave(1,"0","Setpoint%PISLoop2",DriveVolts,Interpolation*InterpSign))+","
			Setpoint = DriveVolts[0]
		endif
	endif
	
	//set the voltage
	ErrorStr += num2str(ir_WriteValue("A%Output",LithoBias))+","
	...
	Note/K DriveY		//thought this might be screwing up Todd, but I think he is fine with the note....

	//these drive the X & Y stages
	ErrorStr += num2str(td_xSetOutWavePair(0,"0","Setpoint%PISLoop3",DriveX,"SetPoint%PISloop4",DriveY,Interpolation*InterpSign))+","
	//this calls the next part	
	//this calls the next part	
	ErrorStr += IR_XSetInWavePair(0,"0","Fast%Input",Deflection,"ZSensor",Height,"LithoRamp("+num2str(LithoCount+1)+")",Decimation)
	
	

	
	Variable A, nop = 3
	Variable UpdateXPT = 0
	String ChannelName, ADCName, ADCList = "", ChanList = ""
	for (A = 0;A < nop;A += 1)
		ChannelName = GTS("LithoChannel"+Num2str(A))
		ChanList += ChannelName+";"
		if (StringMatch(ChannelName,"Lateral"))
			ADCName = num2char(65+A)+"%Input"
			if (A)
				UpdateXPT += 2
			endif
		elseif (StringMatch(ChannelName,"UserIn*"))
			ADCName = StringFromList(A,"A%Input;B%Input;",";")
			UpdateXPT += 2^A
	...
	String UserStr
	if (UpdateXPT & 1)
		UserNum = GetEndNum(StringFromList(0,ChanList,";"))
		XPTPopupFunc("InAPopup",7+UserNum,"In"+num2str(UserNum))
	endif
	if (UpdateXPT & 2)
		UserNum = GetEndNum(StringFromList(1,ChanList,";"))
		UserStr = "In"+num2str(UserNum)
		if (IsNan(UserNum))
			//lateral on B
			UserStr = "Lateral"
			UserNum = -1
		endif
		XPTPopupFunc("InBPopup",7+UserNum,UserStr)
	endif
	if (UpdateXPT)
		XPTButtonFunc("WriteXPT")
	endif
	
	ErrorStr += IR_XSetInWavePair(1,"0",StringFromList(0,ADCList,";"),Input0,StringFromList(2,ADCList,";"),Input2,"",Decimation)
	ErrorStr += IR_XSetInWave(2,"0",StringFromList(1,ADCList,";"),Input1,"",Decimation)
...
//print td_RV("ZSensor")*-GV("ZLVDTSens"),GetFuncName()
	...
	for (A = 0;A < nop;A += 1)
		...
		FastOp DisplayWave = (Scale)*InputWave
...
//DoUpdate
	...
	if (Lithoindex >= DimSize(YLitho,0))						//if the index is past the Wave, we are through
		...
		if (PreSnap)
			...
			//Revert the setpoint.....
			if (imagingMode == 1)
				MainSetVarFunc("SetPointSetVar_0",GV("LithoSetpointVolts"),"",":Variables:MasterVariablesWave[%AmplitudeSetPointVolts]")
			else
				MainSetVarFunc("SetPointSetVar_0",GV("LithoSetpointVolts"),"",":Variables:MasterVariablesWave[%DeflectionSetPointVolts]")
			endif
		endif
		return(0)
	endif
	...
	Variable XDist, YDist, XSpeed, YSpeed, XTime, YTime
	XDist = abs(imag(oldBoth)/XLVDTSens-imag(newBoth)/XLVDTSens)
	YDist = abs(real(oldBoth)/YLVDTSens-real(newBoth)/YLVDTSens)
	XSpeed = (ScanSpeed/(XLVDTSens*4))
	YSpeed = (ScanSpeed/(YLVDTSens*4))
	XTime = XDist/XSpeed
	YTime = YDist/YSpeed
	...
	Variable DoX = MakeRamp(0,"Setpoint%PISLoop3",imag(oldBoth)/XLVDTSens,imag(newBoth)/XLVDTSens,XSpeed,1,rampInterpX)	//now with a 5um/S lowest speed
	Variable DoY = MakeRamp(1,"Setpoint%PISLoop4",real(oldBoth)/YLVDTSens,real(newBoth)/YLVDTSens,YSpeed,1,rampInterpY)
	...
	if (DoX || DoY)										//do we need to ramp?
		...
		if (Snapping)
			...
			ErrorStr += num2str(td_xSetOutWave(2,"0","Setpoint%Pisloop2",Dummywave,RampInterp))+","
		endif
		//call actuallitho after the ramp
		ErrorStr += IR_XSetInWave(0,"0","Fast%Input",DummyWave,"ActualLitho("+num2str(LithoCount)+")",rampInterp)
		if (SnappyTrigger)
			ErrorStr += num2str(td_WriteString("3%Event","Clear"))+","
			ErrorStr += num2str(td_WriteString("2%Event","Once"))+","
		endif
		ErrorStr += num2str(td_WriteString("0%Event","Once"))+","
	else
		ErrorStr += num2str(ir_WriteValue("Setpoint%PISloop3",imag(newBoth)/XLVDTSens))+","
		ErrorStr += num2str(ir_WriteValue("Setpoint%PISloop4",real(newBoth)/YLVDTSens))+","
...
Function MakeLithoPanel(var)		//make the litho panel
	...
	if (2^bit & Control1Bit)
		ControlName = "LithoModePopup"+TabStr
		UpdatePopup(GraphStr,ControlName,"Litho Mode",45,CurrentTop,"LithoPopupFunc","\""+ImagingModes+"\"",GV("ImagingMode")+1,Enab)
...
Function LithoPopupFunc(ctrlName,popNum,popStr)
	String ctrlName, popStr		//PopStr not used
	Variable popNum
	
	
	Variable ImagingMode = PopNum-1
	if (Strlen(PopStr))
		MainPopupFunc("ImagingModePopup_0",ImagingMode+1,"")
	endif
...
//		PV("LithoSlowScanSize",DimSize(ImageWave,1)*DimDelta(ImageWave,1)+DimOffset(ImageWave,1))		//grab the image size
	...
	if (Control1Bit & 2^bit)
		...
		Slider $ControlName,win=$GraphStr,proc=LithoSliderFunc,thumbColor=(Red,Green,Blue)		//Variable=root:Packages:MFP3D:Main:Variables:LithoVariablesWave[%LithoSize]
	...
	if (Control1Bit & 2^bit)
		...
		Slider $ControlName,win=$GraphStr,proc=LithoSliderFunc,side=2,thumbColor=(Red,Green,Blue)	//Variable=root:Packages:MFP3D:Main:Variables:LithoVariablesWave[%LithoSize]
	...
	if (Control1Bit & 2^bit)
		...
		Slider $ControlName,win=$GraphStr,proc=LithoSliderFunc,thumbColor=(Red,Green,Blue)		//Variable=root:Packages:MFP3D:Main:Variables:LithoVariablesWave[%LithoXOffset]
...
Function ClearLithoGroups()
	...
	for (A = 0;A < nop;A += 1)
		...
		if (WhichListItem("GroupDisplayY", RemList,";",0) > -1)
...
Function SetAngle(newValue)		//this increments the size gain from its old value to a new one
	...
	ErrorStr += num2str(td_SetRamp(5,"cos%ScanEngine",0,cos(newValue*pi/180),"sin%ScanEngine",0,sin(newValue*pi/180),"",0,0,"SetDisable(0)"))+","
...
Function SetSizeGain(newValue)		//this increments the size gain from its old value to a new one
	...
	if (GV("ScanMode") == 0)
		ErrorStr += num2str(td_SetRamp(5,"XGain%ScanEngine",0,newValue/GV("XLVDTSens")/(.8*20),"YGain%ScanEngine",0,newValue/GV("YLVDTSens")/(.8*20),"",0,0,"SetDisable(0)"))+","
	else
		ErrorStr += num2str(td_SetRamp(5,"XGain%ScanEngine",0,newValue/GV("XPiezoSens")/(.8*160),"YGain%ScanEngine",0,newValue/GV("YPiezoSens")/(.8*160),"",0,0,"SetDisable(0)")	)+","
	endif
...
Function SetOffset(which,newX,newY)		//this increments the offset from its old value to a new one
	...
	if (GV("ScanMode") == 0)
		if (1 & which)								//which = 1 is Y
			...
			oldX = GV("OldXOffset")/xLVDTSens	//get the old offset
		...
		if (2 & which)
			...
			oldY = GV("OldYOffset")/yLVDTSens
			yChanStr = "YOffset%ScanEngine"
			PV("OldYOffset",newY)		//save the new values
		endif
		withdraw = (abs(newY-oldY*YLVDTSens)*3 > GV("ScanSize")) || (abs(newX-oldX*xLVDTSens)*3 > GV("ScanSize"))
	else
		if (1 & which)								//which = 1 is Y
	...
	Variable laterAction = 0
	if (withdraw)
		if (GV("ImagingMode") == 1)
			laterAction = 2
			ErrorStr += num2str(ir_WriteValue("IGain%PISLoop2",-100))+","
			ErrorStr += num2str(ir_WriteValue("Setpoint%PISLoop2",10))+","
		else
			laterAction = 1
			ErrorStr += num2str(ir_WriteValue("IGain%PISLoop2",100))+","
			ErrorStr += num2str(ir_WriteValue("Setpoint%PISLoop2",-10))+","
	...
	if (GV("ScanMode") == 0)
		ErrorStr += num2str(td_SetRamp(3,xChanStr,0,newX/(xLVDTSens*20)*GV("XScanDirection")+GV("XLVDTOffset")/20,yChanStr,0,newY/(YLVDTSens*20)*GV("YScanDirection")+GV("YLVDTOffset")/20,"",0,0,"FinishSetOffset("+num2str(laterAction)+")"))+","
	else
		ErrorStr += num2str(td_SetRamp(3,xChanStr,0,-newX/(xPiezoSens*160)*GV("XScanDirection"),yChanStr,0,-newY/(YPiezoSens*160)*GV("XScanDirection"),"",0,0,"FinishSetOffset("+num2str(laterAction)+")"))+","
	endif
...
Function FinishSetOffset(laterAction)
	Variable laterAction		

	SetDisable(0)
	String ErrorStr = ""
	//set the setpoint first, so the acumulator has a chance to get back to normal
	//before the gains are jacked back up.
	if (laterAction == 2)
		ErrorStr += num2str(ir_WriteValue("Setpoint%PISLoop2",GV("AmplitudeSetpointVolts")))+","
		ErrorStr += num2str(ir_WriteValue("IGain%PISLoop2",-GV("IntegralGain")*100))+","
	elseif (laterAction == 1)
		ErrorStr += num2str(ir_WriteValue("Setpoint%PISLoop2",GV("DeflectionSetpointVolts")))+","
		ErrorStr += num2str(ir_WriteValue("IGain%PISLoop2",GV("IntegralGain")*100))+","
	endif
...
//anded multi state button support 060314
	...
	Variable Mode = GV("LithoUseBias")
	Variable UseWave = GV("LithoUseWave")		//do we have a setpoint wave?
	
	String ControlList = "LithoBiasSetVar;"
	String OtherControlList = "LithoSetpointVoltsSetVar;"
...
Function GhostLithoStepPanel()
	...
	Variable Mode = GV("LithoStepUseBias")
	Variable UseWave = GV("LithoStepUseWave")		//do we have a setpoint wave?
	
	String ControlList = "LithoStartVoltsSetVar;"
	String OtherControlList = "LithoEndVoltsSetVar;"
	ControlList = ListMultiply(ControlList,TabStr,";")
	OtherControlList = ListMultiply(OtherControlList,TabStr,";")
...
Function PutLithoOnEveryOne(DoCheck)
	...
	for (A = 0;A < nop;A += 1)
		...
		if (WhichListItem("YLitho", TraceList,";",0) < 0)
...
Function PutOnXYSpot(NumLimit)
	...
	for (A = 0;A < nop;A += 1)
		...
		TraceList = ARTraceNameList(GraphStr,"YPoint*","*","*",DataFolder,";",1)
	...
	if (FVW[%ShowXYSpot][0] == 1)
		if (!ScanStatus)
			...
			if ((WaveExists(YPoint) == 1) && (WaveExists(XPoint) == 1))
				OldX = XPoint[0]
				OldY = YPoint[0]
				tempY = (td_ReadValue("YSensor")-OMVW[%YLVDTOffset][0])*OMVW[%YLVDTSens][0]-RVW[%YOffset][0]//+LVW[%LithoScanSize]/2
				tempX = (td_ReadValue("XSensor")-OMVW[%XLVDTOffset][0])*OMVW[%XLVDTSens][0]-RVW[%XOffset][0]//+LVW[%LithoScanSize]/2
...
Function LithoXPTFunc(IsStep)
	...
	Variable UseBias
	if (IsStep)
		UseBias = GV("LithoStepUseBias")
	else
		UseBias = GV("LithoUseBias")
	endif

	if (!GV("DontChangeXPT"))
		if (UseBias)
			if (ImagingMode)
				LoadXPTState("ACVoltLitho")
			else
				LoadXPTState("DCVoltLitho")
			endif
		else
			if (ImagingMode)
				LoadXPTState("ACLitho")
			else
				LoadXPTState("DCLitho")
			endif
		endif
	endif
...
Function ReadGDSData(DataType,FileRef,ByteOrder)
...
Function ReadElement(Counter,FileRef)
...
Function ScaleGDSnop(DataLength,DataVar)
...
Function/S ConvertBitArray(Val)
	Variable Val
	String output = ""
...
Function/S RecType2Str(RecordType,DataType)			//PASS BY REFERENCE!!!!!!!!!!!!!!!!!!!
...
Function ReadGDSStructure(FileRef,Counter)
	Variable FileRef
	Variable counter
	Fstatus FileRef
	if (v_flag == 0)
...
Function IsElement(ElementType)
	String ElementType
	Variable output = 0
...
//
//Function PlotGDSData2()
//
//	String SavedDataFolder = GetDataFolder(1)
//	String DataFolder = GetDF("GDSRoot")
//	SetDataFolder DataFolder
//	String WlistY = WaveList("GDSdataY*",";","")
//	String WlistX = WaveList("GDSdataX*",";","")
//	
//	
//	if (strlen(WlistY) == 0)
//		return 0
//	endif
//	String GraphName = "GDSPlot"
//	DoWindow/K $GraphName
//	Display/k=1/N=$GraphName
//	Variable i, Stop = ItemsInList(WlistY,";")
//	for (i = 0;i < Stop;i += 1)
//		AppendToGraph/W=$GraphName $StringFromList(i,WlistY,";") vs $StringFromList(i,WlistX,";")
//	endfor
//	SetDataFolder SavedDataFolder
//End //PlotGDSData2

//
//Function EditGDSData()
//
//	String SavedDataFolder = GetDataFolder(1)
//	String DataFolder = GetDF("GDSRoot")
//	SetDataFolder DataFolder
...
//		return 0
//	endif
//	String GraphName = "GDSTable"
//	DoWindow/K $GraphName
//	Edit/k=1/N=$GraphName
//	Variable i, Stop = ItemsInList(WlistY,";")
//	for (i = 0;i < Stop;i += 1)
//		AppendToTable $StringFromList(i,WlistY,";"),$StringFromList(i,WlistX,";")
//	endfor
//	SetDataFolder SavedDataFolder
//End //EditGDSdata


Function ScaleGDSdata()
	String SavedDataFolder = GetDataFolder(1)
	String DataFolder = GetDF("GDSRoot")
	SetDataFolder DataFolder
...
OverRide Function CleanGDSdir()						//this function is in the GDS text version.
	String SavedDataFolder = GetDataFolder(1)
	String DataFolder = GetDF("GDSRoot")
	SetDataFolder DataFolder
	String Wlist = WaveList("GDSdata*",";","")
...
Function/S GDSDateSlave(DateStr)
	String DateStr
	String output = ""
	Variable i, Stop = ItemsInList(DateStr,",")
	if (Stop != 6)
...
//	String Slist = StringList("GDSString*",";")
//	Variable i, Stop = ItemsInList(Wlist,";"), Jstop = ItemsInList(Slist,";")
//	if ((Stop == 0) || (Jstop == 0))
//		return 0
//	endif
//	
//	Make/N=(Stop) /O /T GDSWaveList
//	Make/N=(Jstop) /O /T GDSStringList
//	Make/O/N=(Jstop) GDSStringSel
//	Make/O/N=(Stop) GDSWaveSel
//	Wave/T GDSWaveList = GDSWaveList
//	Wave/T GDSStringList = GDSStringList
//	Wave GDSStringSel = GDSStringSel
//	Wave GDSWaveSel = GDSWaveSel
//	
//	
//	FastOp GDSWaveSel = 0
//	FastOp GDSStringSel = 0
//	
//	for (i = 0;i < Stop;i += 1)
//		GDSWaveList[i] = StringFromList(i,Wlist,";")
//	endfor
//	
//	for (i = 0;i < Jstop;i += 1)
//		GDSStringList[i] = StringFromList(i,Slist,";")
//	endfor
//	
//	
//	DoWindow/K $GraphName
//	Display/k=1/N=$GraphName /W=(106.2,74.6,580.2,458.6)
//	ControlBar/W=$GraphName 262
//	ModifyGraph/W=$GraphName wbRGB=(0,3072,13056), cbRGB=(0,39168,0)
//	
//	String C_con = "WaveListBox"
//	ListBox $C_con, win=$GraphName, Title="GDS Wave", Proc=ShowGDSWave, ListWave=GDSWaveList, SelWave=GDSWaveSel, mode=4, pos={10,30}, size={100,200}
//	TitleBox WaveListTitle, win=$GraphName, Title="GDS Wave", pos={10,7}, size={100,20}
//	
//	c_con = "StringListBox"
//	ListBox $C_con, win=$GraphName, Title="GDS String", Proc=ShowGDSString, ListWave=GDSStringList, SelWave=GDSStringSel, mode=4, pos={140,30}, size={100,200}
//	TitleBox StringListTitle, win=$GraphName, Title="GDS String", pos={140,7}, size={100,20}
//	
//	Variable spc = 25, off = 10
//	for (i = 0;i < 9;i += 1)
//		c_con = "GDSSetVar" + num2str(i)
//		SetVariable $C_con, win=$GraphName, title="String " + num2str(i), disable=1, size={500,20}, pos={250,spc * i + off}
//	endfor
//	
//	Button GDSKillButton, win=$GraphName, pos={13,236}, size={80,20}, proc=KillGDSWave, title="Kill Waves"
//	Button GDSEditButton, win=$GraphName, pos={113,236}, size={80,20}, proc=EditGDSWaves, title="Edit Waves"
//	Button GDSBuildButton, win=$GraphName, pos={213,236}, size={130,20}, proc=ConvertGDSButton, title="Convert 2 Litho"
//	SetDataFolder SavedDataFolder
//	
//	
//End //GDSbrowser
...
Function ConvertGDSButton(CtrlName)
	String CtrlName		//not used
	ConvertGDS2Litho()
End //ConvertGDSButton


Function ShowGDSWave(ctrlName,row,col,event)
	String ctrlName     //Not Used
	Variable row        //Used
	Variable col        //Not Used
	Variable event      //Used
	...
	String GraphName = cGDSPN
	Variable Xl, Xh, Yl, Yh
	GetAxes(GraphName,Xl,XH,Yl,YH)
	...
	Variable i, Stop = DimSize(GDSWaveList,0), Red, Blue, Green, Cnt = 0
	for (i = 0;i < Stop;i += 1)
		GDSInd = GetEndNum(GDSWaveList[i])
		Sname = "GDSString" + num2str(GDSind)
		ind = Find1TWave(GDSStringList,Sname)
		GDSStringsel[ind] = GDSWaveSel[i]
		if (GDSWaveSel[i] != 0)
			Xname = "GDSdataX" + num2str(GDSind)
			AppendToGraph/W=$GraphName $GDSWaveList[i] vs $Xname
			ARColorTable(Cnt,Red,Blue,Green)			//PASS BY REFERENCE
			ModifyGraph rgb($GDSWaveList[i])=(Red,Blue,Green)
			Cnt += 1
		endif
	endfor
	
//	SetAxis/Z/W=$GN Left, Yl, Yh
//	SetAxis/Z/W=$GN Bottom, Xl, Xh

	if (numtype(Row) == 0)
		ShowGDSString("",NaN,0,4)
	endif

	ModifyGraph lsize=2
	ModifyGraph tick=1, fStyle=1, axThick=2
...
Function ShowGDSString(ctrlName,row,col,event)
	String ctrlName     //Not Used
	Variable row        //Used
	Variable col        //Not Used
	Variable event      //Used
	...
	String Yname, c_con
	Variable GDSInd, ind, Cnt = 0, cntLim = 9
	
	Variable i, Stop = DimSize(GDSStringList,0)
	for (i = 0;i < Stop;i += 1)
		if (numtype(Row) == 0)
			GDSInd = GetEndNum(GDSStringList[i])
			Yname = "GDSdataY" + num2str(GDSind)
			ind = Find1TWave(GDSWaveList,Yname)
			GDSWavesel[ind] = GDSStringSel[i]
		endif
		if (GDSStringSel[i] != 0)
			if (Cnt < cntLim)
				c_con = "GDSsetvar" + num2str(Cnt)
				SetVariable $C_con, win=$GraphName, value=$DataFolder+GDSStringList[i][0], disable=0, title=GDSStringList[i]
				Cnt += 1
			endif
		endif
	endfor
	...
	for (i = cnt;i < cntLIm;i += 1)
		c_con = "GDSsetvar" + num2str(i)
		SetVariable $C_con, win=$GraphName, disable=1
	endfor
...
Function ClearGDSPlot()
	...
	Variable i, Stop = ItemsInList(Wlist,";")
	for (i = 0;i < Stop;i += 1)
		RemoveFromGraph/W=$GraphName /Z $StringFromList(i,Wlist,";")
	endfor
	
End //ClearGDSPlot


Function KillGDSWave(CtrlName)
	String CtrlName //Not Used
	...
	Wave/T GDSStringList = GDSStringList
	Wave GDSStringSel = GDSStringSel
	Variable i, Stop = DimSize(GDSWaveList,0)
	
	
	String Xname, Sname
	Variable GDSInd, ind
	
	Make/N=(DimSize(GDSStringsel,0))/O Death2String
	Wave Death2String = Death2String
	FastOp Death2String = 0			//I don't trust overwrite.
	...
	for (i = 0;i < Stop;i += 1)
		if (GDSWaveSel[i] > 0)
			...
			KillStrings/Z $Sname
			KillWaves/Z $GDSWaveList[i], $Xname
	...
	LogicalIndexText(GDSStringList,Death2String)
	LogicalIndex(GDSStringSel,Death2String)
	
	LogicalIndexText(GDSWaveList,GDSWaveSel)
	Duplicate/O GDSWaveSel, TempWave
	LogicalIndex(GDSWaveSel,TempWave)
	
	KillWaves TempWave, Death2String
...
Function EditGDSWaves(CtrlName)
	String CtrlName	//Not used


	String GraphName = cGDSTN
	String DataFolder = GetDF("GDSRoot")
	String SavedDataFolder = ActuallyQuoteName(GetDataFolder(1))
	...
	DoWindow/K $GraphName
	Edit/K=1/W=(151.2,87.2,412.8,430.4)/N=$GraphName
	String Xname
	Variable GDSInd
	...
	for (i = 0;i < Stop;i += 1)
		if (GDSWaveSel[i]>0)
			GDSInd = GetEndNum(GDSWaveList[i])
			Xname = "GDSdataX" + num2str(GDSind)
			AppendToTable $GDSWaveList[i], $Xname
		endif
	endfor
...
Function ConvertGDSLithoPointDensity(Fname)
	String Fname
...
Function LithoInterp(nop,Type,Xdata,Ydata,Xdest,Ydest,Xdiff,Ydiff,XYDiff,Xsub,Ysub)
	Variable nop, Type
...
//Function CheckLithoSpacing(Fname,Start,thresh)
//	String Fname
//	Variable Start
//	Variable Thresh
//	
//	//this is a debuggin function to make sure that ConvertGDSLithoPointDensity
//	//Did it's job right.
//	
//	String SD = ACtuallyQuoteName(GetDataFolder(1))		//not used yet.
//	String DF = "Root:Packages:MFP3D:Litho:Groups:"
//	
//	Wave/Z Xdata = $DF+"'X"+Fname+"'"
//	Wave/Z Ydata = $DF+"'Y"+Fname+"'"
//	
//	if ((WaveExists(Xdata) == 0) || (WaveExists(Ydata) == 0))
//		print "can not find Litho data: "+Fname
//		return(0)
...
//	IndexWave[0] = -1
//	
//	print Fname+" has "+num2str(numpnts(IndexWave)-1)+" groups"
//	
//	Wave Xsub = $LocalWave(0)
//	Wave Ysub = $LocalWave(0)
//	Wave Xdiff = $localWave(0)
...
//	Wave XYDiff = $LocalWave(0)
//	
//	Redimension/D Xdiff,Ydiff,XYDiff,Xsub,Ysub
//	
//	Variable A, nop = DimSize(IndexWave,0)
//	for (A = Start;A < nop-1;A+=1)
//	
//		Redimension/N=(IndexWave[A+1]-IndexWave[A]-1) Xsub,Ysub
//		Redimension/N=(IndexWave[A+1]-IndexWave[A]-2) XDiff,YDiff,XYdiff
//		
//		Xsub[] = Xdata[p+IndexWave[A]+1]
//		Ysub[] = Ydata[p+IndexWave[A]+1]
//		
...
//			XYDiff = sqrt(Xdiff^2+YDiff^2)
//			WaveStats/Q XYDiff
//			Variable compar = v_max/v_min
//			if (Compar > Thresh)					//Bad, show me why.
//				print "****Group: "+num2str(A)+" is Bad; Compair: "+num2str(compar)
//				//Edit/k=1 Xdiff,YDiff,XYDiff
//				Dowindow/K CompGraph
//				Display/K=1 Ysub vs Xsub
//				DoWindow/C CompGraph
//				ModifyGraph mode=4,marker=19
//				DoWindow/H
//				return(0)
//			else
//				print "Group: "+num2str(A)+" is Good; Comapir: "+num2str(compar)
//			endif
//			InsertPoints cntX,1,CompWave
//			CompWave[cntX] = Compar
//			cntX += 1
//			
//		else
//			print "Group: "+num2str(A)+" has "+num2str(numpnts(Xsub))+" points"
//			if (numpnts(Xsub)==0)
//				print "******************************************************"
//			endif
//		endif
...
//	endfor
//	
//	KillWaves IndexWave,Xsub,Ysub
//	
//	ARHistogram(CompWave,"CheckLithoSpacingHisto",0)
//	ModifyGraph log(left)=1
//	
//End //CheckLithoSpacing
...
Function DoLithoSnapFunc(CtrlName)
	...
	StrSwitch (CtrlName)
		...
		case "DoLithoSnap":
			PV("LithoSnap",1)
			//LithoPopupFunc("LithoModePopup_0",1,"Deflection")
...
Function PreLithoSnap()
	...
	if (ImagingMode)		//AC
		...
		OtherSetPoint = -12
		ParmName = "DeflectionSetpointVolts"
	endif
	SetPoint = GV(ParmName)
...
Function InitVelocityScan(CtrlName)
	...
	StrSwitch (CtrlName)
		case "VelocityLoadImage":
			...
			if (DimSize(MasterImage,2) > 1)
				...
				Duplicate/O MasterWave,M_RGB2Gray
			endif
	...
	StrSwitch (CtrlName)
		case "VelocityDoScan":
			...
			SetDataFolder(SavedDataFolder)
			ErrorStr += IR_XSetInWavePair(0,"0","ZSensor",ZLVDT,"Z%output",Height,"",OtherInterp)
			ErrorStr += IR_XSetInWavePair(1,"0","XSensor",XLVDT,"YSensor",YLVDT,"",OtherInterp)
			ErrorStr += IR_XSetInWave(2,"0","Fast%Input",Deflection,"VelocityCallback()",OtherInterp)
...
Function VelocityCallback()
	...
	FastOp XLVDT = (XLVDTSens)*XLVDT
	FastOp YLVDT = (YLVDTSens)*YLVDT
...
Function PlanE(GreyImage,FastScanWave,SlowScanWave,MinVel,MaxVel,ScanSize)
	...
	Variable XLVDTSens = GV("XLVDTSens")
	Variable YLVDTSens = GV("YLVDTSens")
	Variable FastScanSize = ScanSize/XLVDTSens
	Variable SlowScanSize = ScanSize/YLVDTSens
	...
	Variable InterpVal = round(1e5/PPS)
	PPS = 1e5/InterpVal
	...
	for (B = 0;B < ScanLines;B += 1)
		...
		FastOP InterpDist = (1/XLVDTSens)*InterpDist		//Vs
	...
	FastOP FastScanWave = (-ScanSize/2/XLVDTSens)+FastScanWave
	FastOp SlowScanWave = (-ScanSize/2/YLVDTSens)+SlowScanWave
...
Function BufferScan(FastWave,SlowWave,InterpVal)
	...
	String ErrorStr = ""
	ErrorStr += num2str(td_WriteString("1%Event","Clear"))+","
	ErrorStr += num2str(td_WriteString("2%Event","Clear"))+","
	ErrorStr += num2str(td_WriteString("0%Event","Clear"))+","
	ErrorStr += num2str(td_StopOutWaveBank(-1))+","
	...
	SetDimLabel 0,0,$"1%Event",EvenWave
	SetDimLabel 0,1,$"2%Event",EvenWave
	EvenWave[0] = 1
	EvenWave[1] = 0
	SetDimLabel 0,0,$"2%Event",OddWave
	SetDimLabel 0,1,$"1%Event",OddWave
	OddWave[0] = 1
	OddWave[1] = 0
	...
	if (ScanParms[%NumOfChunks][0] > 1)
		...
		ErrorStr += num2str(td_xSetOutWave(0,"0,0","Dummy%output",DummyWave,InterpVal))+","
		ErrorStr += num2str(td_WriteString("OutWave0StatusCallback","BufferScanCallback(0)"))+","
		ErrorStr += num2str(td_xSetOutWavePair(1,"1","Setpoint%PISLoop3",FastA,"Setpoint%PISLoop4",SlowA,InterpVal))+","
		ErrorStr += num2str(td_xSetOutWavePair(2,"2","Setpoint%PISLoop3",FastB,"Setpoint%PISLoop4",SlowB,InterpVal))+","
		
	else
		ErrorStr += num2str(td_xSetOutWavePair(1,"0","Setpoint%PISLoop3",FastA,"Setpoint%PISLoop4",SlowA,InterpVal))+","
	endif
...
Function BufferScanCallback(IsEven)
	...
	String ErrorStr = ""
	ErrorStr += num2str(td_xSetOutWavePair(Bank,Event,"SetPoint%PISLoop3",SmallFast,"Setpoint%PISLoop4",SmallSlow,InterpVal))+","
...
Function VelocityBufferScan(FastWave,SlowWave,InterpVal)
	...
	Variable xScanOffset = (GV("XLVDTOffset")+xOffset/xLVDTSens)
	Variable yScanOffset = (GV("YLVDTOffset")+yOffset/yLVDTSens)
	...
	String ErrorStr = ""
//	ErrorStr += num2str(ir_StopPISLoop(0))+","
//	ErrorStr += num2str(ir_StopPISLoop(1))+","
	...
	//set the X and Y feedback loops
	ErrorStr += num2str(ir_SetPISLoop(3,"1","XSensor",NaN,0,XIgain,0,"X%Output"))+","
	//best not to use secret gain for simple ramps
	ErrorStr += num2str(ir_SetPISLoop(4,"1","YSensor",NaN,0,YIgain,0,"Y%Output"))+","
	//this is the X & Y offset
...
//	ErrorStr += num2str(td_WriteString("0%Event","Clear")
	...
	ErrorStr += num2str(td_SetRamp(1,"setpoint%PISloop3",0,FastWave[0],"Setpoint%PISLoop4",0,SlowWave[0],"",0,0,"td_WriteString(\"1%Event\",\"Once\");td_WriteString(\"0%Event\",\"Set\")"))+","
...
Function/S GetLithoChannels(ChanNum)
	...
	switch (ChanNum)
		...
		case 1:
			Output = "Current;Lateral;"+UserList
			break
...
function UpdateImageWrite(scanLines,scanPoints,decimation,scanDown,lithoBiasOff, whichCall)
	...
	if (whichCall)
		...
		td_xSetOutWavePair(2,"update","A%Output",ImageDrive,"Dummy%Output",ImageDrive,Decimation)
